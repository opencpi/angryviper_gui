...
public class OCSPropertyXmlListBinding extends StandardXmlListBindingImpl {
	...
	protected void initBindingMetadata() {
		...
		if (this.xmlElementNames.length == 2) {
			...
			/*
			this.xmlElementNames[2] = createQualifiedName("whatever", xmlNamespaceResolver);
			if (this.xmlElementNames[2] == null) {
				final ElementType type = this.modelElementTypes[2];
				this.xmlElementNames[2] = createDefaultElementName(type, xmlNamespaceResolver);
			}
			*/
		}
	}
