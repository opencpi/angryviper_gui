...
import av.proj.ide.ocs.Port;
import av.proj.ide.ocs.PortLower;

public class OCSPortXmlListBinding extends StandardXmlListBindingImpl {
	...
	protected void initBindingMetadata() {
		...
		this.modelElementTypes[0] = new ElementType(Port.class);
		this.modelElementTypes[1] = new ElementType(PortLower.class);
		this.xmlElementNames = new QName[ this.modelElementTypes.length ];            

		if (this.xmlElementNames.length == 2) {
			this.xmlElementNames[0] = createQualifiedName("Port", xmlNamespaceResolver);
			...
			this.xmlElementNames[1] = createQualifiedName("port", xmlNamespaceResolver);
