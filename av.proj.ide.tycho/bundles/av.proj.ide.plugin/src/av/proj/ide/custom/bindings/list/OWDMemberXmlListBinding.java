...
import av.proj.ide.owd.Member;

public class OWDMemberXmlListBinding extends StandardXmlListBindingImpl {
	...
	protected void initBindingMetadata() {
		...
		this.modelElementTypes[0] = new ElementType(Member.class);
		this.modelElementTypes[1] = new ElementType(Member.class);
		this.xmlElementNames = new QName[ this.modelElementTypes.length ];            

		if (this.xmlElementNames.length == 2) {
			this.xmlElementNames[0] = createQualifiedName("Member", xmlNamespaceResolver);
			...
			this.xmlElementNames[1] = createQualifiedName("member", xmlNamespaceResolver);
