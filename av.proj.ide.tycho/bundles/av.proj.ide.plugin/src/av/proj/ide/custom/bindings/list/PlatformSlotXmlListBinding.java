...
public class PlatformSlotXmlListBinding extends PlatformSpecPropertyXmlListBinding {
	...
	protected void initBindingMetadata() {
		...
		this.modelElementTypes[0] = new ElementType(Slot.class);
		this.modelElementTypes[1] = new ElementType(Slot.class);
		this.xmlElementNames = new QName[ this.modelElementTypes.length ];            

		if (this.xmlElementNames.length == 2) {
			this.xmlElementNames[0] = createQualifiedName("Slot", xmlNamespaceResolver);
			...
			this.xmlElementNames[1] = createQualifiedName("slot", xmlNamespaceResolver);
