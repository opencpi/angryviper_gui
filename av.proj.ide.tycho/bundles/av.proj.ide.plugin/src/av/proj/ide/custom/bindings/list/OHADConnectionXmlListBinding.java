...
public class OHADConnectionXmlListBinding extends StandardXmlListBindingImpl {
	...
	protected void initBindingMetadata() {
		...
		this.modelElementTypes[0] = new ElementType(Connection.class);
		this.modelElementTypes[1] = new ElementType(Connection.class);
		this.xmlElementNames = new QName[ this.modelElementTypes.length ];            

		if (this.xmlElementNames.length == 2) {
			this.xmlElementNames[0] = createQualifiedName("Connection", xmlNamespaceResolver);
			...
			this.xmlElementNames[1] = createQualifiedName("connection", xmlNamespaceResolver);
