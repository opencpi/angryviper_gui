...
public class NoControlListBinding extends DelimitedListBindingImpl {
	...
	public NoControlListBinding() {
		this.path = new XmlPath( "@noControl", NAMESPACE_RESOLVER );
	}
	...
	protected String readListString() {
	    ...
	    if( listXmlNode == null ) {
	        XmlPath tmpPath = new XmlPath("@NoControl", NAMESPACE_RESOLVER);
	        listXmlNode = parentXmlElement.getChildNode(tmpPath , false);
	        
	        if (listXmlNode == null) {
	        	tmpPath = new XmlPath("@nocontrol", NAMESPACE_RESOLVER);
		        listXmlNode = parentXmlElement.getChildNode(tmpPath , false);
