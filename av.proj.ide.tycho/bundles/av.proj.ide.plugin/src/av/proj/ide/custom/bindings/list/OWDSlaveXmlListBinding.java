...
import av.proj.ide.owd.rcc.Slave;

public class OWDSlaveXmlListBinding extends StandardXmlListBindingImpl {

	@Override
	protected void initBindingMetadata() {
		/**
		 * SelfNote: Refer to StandardXmlListBindingImp to see all the types of class variables
		 */
		...
		this.modelElementTypes[0] = new ElementType(Slave.class);
		this.modelElementTypes[1] = new ElementType(Slave.class);
		this.xmlElementNames = new QName[ this.modelElementTypes.length ];
		
		/***
		 *This code block is for experimental purpose to see 
		 *how to enforce not to have slave attribute and slave element
		 *at the same time. 
		 */
		/*
        final XmlElement parent = getXmlElement( false );
	    boolean hasSlaveAttribute = false;

		List<org.eclipse.sapphire.modeling.xml.XmlAttribute> list = parent.getAttributes();
        for (org.eclipse.sapphire.modeling.xml.XmlAttribute itr : list){
        	if("Slave".equals(itr.getLocalName())){
        		hasSlaveAttribute = true;
        	};
        	
        }
		*/
        if (this.xmlElementNames.length == 2) {
        	//if(!hasSlaveAttribute){
        	
                //This is where element name for Slave is created - <Slave> </Slave> 
			    this.xmlElementNames[0] = createQualifiedName("Slave", xmlNamespaceResolver);
			    ...
			    this.xmlElementNames[1] = createQualifiedName("slave", xmlNamespaceResolver);
